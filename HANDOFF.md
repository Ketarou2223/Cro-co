...
| 種別 | 内容 |
|---|---|
| ⚠️ 運用 | dev / 本番の SQL マイグレーション適用が手動。035/037/038/039 は両環境で適用確認済み（2026-05-27）。040 は 2026-05-27 にオーナーが dev/prod 手動適用（3本収束の schema 確認は次回）。041（storage バケット作成）は 2026-05-27 に dev/prod へ適用し両環境の `storage.buckets` 一致を確認。新規分は引き続き手動・docs/ARCHITECTURE.md の表で追跡 |
| ✅ 解消（2026-05-27） | dev に storage バケット（profile-images / student-ids）が未作成だった問題。migration 041 で dev/prod 両方に作成（prod 同設定 Private/5MB/image/jpeg+png）。dev/prod を migration ファイルだけで再現可能な状態に到達 |
...
- **2026-05-27**: dev に profile-images / student-ids バケットを作成（prod と完全同設定: Private/`file_size_limit=5242880`/`allowed_mime_types={image/jpeg,image/png}`/`avif_autodetection=false`/`owner=null`/`type=STANDARD`）。**migration 041 として SQL 化**（B-1 採用）。理由: Supabase 公式が `storage.buckets` への直 INSERT を第一級の手段として明記（「Create a bucket」の SQL タブ / self-hosting 移行ガイドの `ON CONFLICT (id) DO NOTHING` 冪等パターン）しており、`auth.users` のような Dashboard 専用テーブルではない。STANDARD バケットは `storage.buckets` の1行で完結し付随メタデータの分散もない。Step A のフィージビリティ調査結果を報告しオーナーが B-1 を選択。dev（新規作成）/prod（既存のため no-op）の両方に冪等 INSERT を適用し、両環境で `storage.buckets` 全カラムが一致することを SELECT で確認。storage の RLS ポリシーは prod/dev とも 0 件（service_role でアップロード＆署名 URL 生成＝RLS バイパスのため不要）。これにより設計判断「dev/prod を migration ファイルだけで完全再現」が storage まで到達。⚠️ **未検証**: dev での service_role 経由アップロード→署名 URL 取得の HTTP 疎通確認は未実施（dev service_role キーがローカルに無く、prod への書き込みは本タスクの禁止事項のため代替不可）。Step 4 の実機テスト、またはオーナーが dev キーで疎通スクリプトを実行して確認する。⚠️ **未確認**: GitHub Branch Protection の現状確認は gh CLI 未インストール・GitHub トークン未設定のため実施できず（保護設定の取得は認証付き admin API が必要）。設定変更は一切行っていない。
