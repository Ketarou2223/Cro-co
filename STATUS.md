...
- 2026-06-05 **ユーザー画面クイック修正 Q-1/Q-2/Q-7/P-2/P-7 完了。** (Q-1) BrowsePage の「TODAY'S LIKES: n」表示を削除・`/api/likes/today-count` クエリも除去。(P-7) いいね在庫ピルを拡大（text-sm px-3 py-1 border-2）・フォーマットを `♡×n` に変更。(Q-2) HomePage ヒーロー黒帯の PC 表示を縮小（md:min-h-0 md:max-h-[220px]）・minHeight/maxHeight を Tailwind クラスに移動。(Q-7) ProfileDetailPage の `window.confirm` ブロック確認ダイアログを除去（サイレント実行に変更）・ChatPage の shadcn AlertDialog ブロック確認ダイアログ＋showBlockDialog state を除去（DropdownMenuItem から直接 handleBlock 呼び出しに変更）・AlertDialog import 群を削除。(P-2) ProfileEditPage のアカウント情報（学籍情報）欄の「必須」バッジ・「変更不可」バッジを削除。tsc -b 0エラー・vite build ✓ 確認済み。⚠️ 実機目視はオーナー Preview 確認。
