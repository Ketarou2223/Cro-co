  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4 pb-16">
      ...
      <Article id="第2条" title="取得する個人情報の項目">
        ...
        <Sub title="(3) 本人確認情報（第4条に基づき特別に取り扱います）">
          <Ul items={[
            '生年月日',
            '学生証の表面の画像',
            '写真付き身分証明書（運転免許証、個人番号カード等）の表面の画像',
          ]} />
        </Sub>
        <Sub title="(4) 利用状況情報">
          <Ul items={[
      ...
      <Article id="第4条" title="本人確認情報の特別取扱い">
        ...
        <Sub title="(1) 取得の目的">
          <p>
            学生証および写真付き身分証明書の表面の画像は、利用者が在籍大学の現役学生かつ18歳以上であることの確認および本サービスの安全性確保のためにのみ利用し、確認後は本条に定める期間内に削除します。これらの画像を除く本人確認情報は、本サービスの改善に利用することがあります。
          </p>
        </Sub>
        <Sub title="(2) 物理削除の時期">
          <p>本サービスは、本名・学籍番号を取得しません。取得する本人確認情報のうち、学生証および写真付き身分証明書の画像は以下のタイミングで物理的に削除します。生年月日は審査完了後も在籍確認・年齢確認の記録として保持します。</p>
          <Ul items={[
            '審査において承認された場合: 承認時点から5日以内に学生証および写真付き身分証明書の画像を削除',
            '審査において不承認となった場合: 不承認時点から31日以内に学生証および写真付き身分証明書の画像を削除',
            '利用者が退会した場合: 退会手続の完了時点で即時に削除',
          ]} />
        </Sub>
        <Sub title="(3) メールアドレスのハッシュ値の保持">
          <p>再登録防止等の目的のため、メールアドレスについてハッシュ値（email ハッシュ）を以下の目的で保持します。</p>
          <Ul items={[
            '利用規約違反または不正登録を行った利用者の再登録防止',
            '法令に基づく要請への対応',
          ]} />
          <p className="mt-1">退会後30日間保持し、利用規約違反による利用停止（BAN）が継続している場合はBAN期間終了まで保持を延長します。保持期間経過後は、定期処理により物理的に削除します。</p>
        </Sub>
        <Sub title="(4) 削除実行の責任">
          <p>
        ...
        <Sub title="(5) 個人番号（マイナンバー）の不取扱い">
          <p>
            写真付き身分証明書として個人番号カード（マイナンバーカード）が提出される場合がありますが、本サービスは個人番号（マイナンバー）を取得、利用または保管しません。個人番号が記載された面の画像が提出された場合、運営者は当該申請を承認せず、当該画像を速やかに削除します。
          </p>
        </Sub>
      </Article>
      ...
      <Article id="第5条" title="各情報の保存期間">
        ...
        <div className="space-y-1.5">
          {[
            { label: 'プロフィール情報・生年月日・利用履歴・通知設定・お問い合わせ情報', value: '退会時に即時削除' },
            { label: '学生証・写真付き身分証明書の画像', value: '審査承認後5日以内・不承認後31日以内・退会時即時削除' },
            { label: 'メールアドレスのハッシュ値', value: '退会後30日間保持（BAN継続中は延長）後に削除' },
      ...
      <Article id="第6条" title="個人情報の安全管理措置">
        ...
        <Sub title="(1) 技術的安全管理措置">
          <Ul items={[
            ...
            '認証情報の暗号化保管',
            '本人確認書類の画像（学生証・写真付き身分証明書）の非公開ストレージでの管理',
          ]} />
        </Sub>
      ...
      <Article id="第11条" title="未成年者の取扱い">
        <p>
          本サービスは、利用者の年齢が18歳以上であることを登録時に確認します。年齢の確認は、生年月日の自己申告に加え、写真付き身分証明書（運転免許証、個人番号カード等）の表面の画像の目視審査により実施します。確認の完了後は、利用者に固有の識別符号（IDおよびパスワード）を付与し、以後は当該識別符号により本人性を確認することをもって、18歳以上であることの確認に代えるものとします。
        </p>
        <p>
      ...
      <Article id="第12条" title="退会および個人情報の削除">
        ...
        <Sub title="(3) 退会後一定期間保持した後に物理削除する情報">
          <p>メールアドレスのハッシュ値（email ハッシュ）は、不正登録防止等の目的のため退会後30日間保持します。ただし、利用規約違反による利用停止（BAN）が継続している場合は、BAN期間が終了するまで保持を延長します。保持期間経過後は、定期処理により物理的に削除します。</p>
        </Sub>
