...
- ユーザーがどのページからも袋小路にならないようにする

### いいね・通知の仕様（2026-05 確定）
- 通常いいね受信上限（5件/日）の対象: 男女マッチ志向の女性のみ
- 同性ペア（男男・女女）は双方無制限
- 足跡経由いいね（via_footprint=true）はカウント外
- 女性が未解放/上限到達状態でも、以下からは男性ユーザーがいいね可能:
  - /footprints（足跡一覧）
  - /likes/received（受け取ったいいね一覧）
  - /notifications 内のプールから飛んだプロフィール
  これらは should_count_quota で false 判定されカウントしない
- 「受信枠制限の対象」は GET /api/profiles の一覧フィルタのみ
  個別プロフィール（GET /api/profiles/{id}）や足跡・いいね経由は制限しない

### マッチタブ vs 通知タブの役割分担
- マッチタブ: 「未確認のいいね・足跡」+「マッチ済みの会話」
  確認後はマッチタブから消える（既読化）
- 通知タブ: 「過去に来たすべてのいいね・足跡」プール
  既読でも残る・いつでもここからいいね返しできる
