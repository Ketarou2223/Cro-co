...
> **AI への注意**: `@ecs.osaka-u.ac.jp` は阪大「工学系」専用ではなく、**学部生・院生全員**の共通メールアドレス（終生有効）。
> 「工学系限定」「数千人規模」と書いたり説明したりしないこと。院生は登録不可などと説明しないこと。

- `@ecs.osaka-u.ac.jp` は大阪大学**学部生・院生全員**の共通メールアドレス（終生有効）
- 対象母数: 学部生・院生（数値未確定のため記載省略）
- 院生も登録可能（2026-06-22 対応済み・学年: 修士1年〜博士3年を選択可能）
...
- ~~`login_history` テーブルは作成済みだが書き込みコードが存在しない~~ → ✅ 解消（2026-06-22・migration 054 で DROP・コード参照ゼロ確認）
