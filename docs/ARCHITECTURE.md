...
[ Supabase ]  PostgreSQL + Auth + Storage
   - Storage: student-ids (Private) / profile-images (Private・2026-05-27 確認)
...
> **prod/dev 差分（解消）**: prod に存在していた migration 外の手動 RLS ポリシー `blocks_delete_own/insert_own/select_own`（dev には無かった）は、`040_normalize_blocks_rls.sql` を dev/prod に適用済み 2026-05-27（オーナー手動適用）で正規化。⚠️ blocks ポリシーが3本に収束した事実の post-apply schema 確認は次回。なお dev には storage バケットが未作成（prod のみ student-ids / profile-images を保有）。
