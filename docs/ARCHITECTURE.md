...
| 054 | drop_login_history | `login_history` テーブルを DROP（migration 019 作成済み・書き込みコードなし・コード参照ゼロ確認）。**dev 適用済み 2026-06-22** / **prod はオーナー手動待ち** |
| 055 | drop_looking_for | `profiles.looking_for` カラムを DROP（フロント/バック参照を全除去後に適用）。**dev 適用済み 2026-06-22** / **prod はオーナー手動待ち** |
| 056 | drop_hash_columns | `profiles.real_name_hash` / `student_number_hash` の DROP 準備（SSoT = identity_block_hashes・カバレッジ 100% 確認済み）。**⚠️ 未適用（dev/prod とも）—— コード参照4箇所除去後にオーナー GO で実施** |
| 057 | add_email_hash_to_identity_block_hashes | `identity_block_hashes.email_hash` カラム追加（NULL 可・部分一意インデックス `uq_ibh_email_hash WHERE email_hash IS NOT NULL`）。Phase A 照合キー移管に必要。**dev 適用済み 2026-06-22** / **prod ⚠️ 未適用 — backend デプロイ後にオーナー手動適用 + `scripts/backfill_email_hash.py` 実行** |
| 058 | allow_null_student_number_hash | `identity_block_hashes.student_number_hash NOT NULL` 制約を DROP NOT NULL で解除。未承認退会ユーザーが email_hash のみで IBH に INSERT できるよう変更（旧制約で未承認退会 IBH 未記録 + 宙吊り4件が発生していた根本原因の一つ）。**dev 適用済み 2026-06-22** / **prod ⚠️ 未適用** |
