...
| マッチ | MatchesPage.tsx | GET /api/matches/, /matches/unread-count, /likes/received?for_match_tab=true, POST /likes/, /likes/dismiss/ | なし | ブロック・match 除外。GET /api/matches/ は last_message/last_activity_at/unread_count を返す（2026-06-23 追加）。非表示はプロフィール詳細から操作（マッチタブから除去済み） |
| いいね受信 | LikesReceivedPage.tsx | GET /api/likes/received, POST /likes/received/confirm, /likes/ | なし | ブロック・match・身バレ防止 除外。viewer が female+score<80 のとき全 liker を blurred=true（退会済み除く）で返しすりガラスプレースホルダー表示・プロフィール遷移無効 |
| 通知 | NotificationsPage.tsx | GET /api/notifications/, /matches/unread-count, /announcements, POST /notifications/{id}/read, /announcements/read | なし | from_user_id ブロック除外。お知らせは get_approved_user のみ（block_utils 不要） |
