...
[ Supabase ]  PostgreSQL + Auth + Storage
   ...
   - pg_cron: like_quota 日次生成 / daily_metrics 日次スナップ
...
- WebSocket は JWT を `Sec-WebSocket-Protocol` ヘッダで受け取り `supabase.auth.get_user` で検証（ws.py・2026-06-06 B-11 で URL クエリ→ヘッダ移行）

### ディレクトリ構造（抜粋）
```
backend/app/
├── main.py                  # FastAPI app・ルーター14本登録・APScheduler・CORS・セキュリティヘッダー・HSTS（prod のみ）・BodySizeLimitMiddleware・**MaintenanceMiddleware（CORS 内側）**・APP_ENV=production 時に /docs /redoc /openapi.json を無効化
│                            # ミドルウェア実行順（外→内）: CORSMiddleware → MaintenanceMiddleware → BodySizeLimitMiddleware → SecurityHeadersMiddleware → ルーター
...
│          **maintenance.py（is_maintenance_on / set_maintenance・15秒 TTL キャッシュ）**
...
└── routers/  health / profile / browse / like / match / message / ws /
              ...
              **maintenance（GET /api/maintenance/status）** / **admin_maintenance（POST/GET /api/admin/maintenance）**
frontend/src/
├── App.tsx                  # 27ルート（lazy + Suspense）
├── lib/ api.ts / supabase.ts / utils.ts / validation.ts / db.ts
├── contexts/ AuthContext.tsx
├── hooks/ useProfile / useChat / usePWAInstall / usePageTitle / useUnreadCount
...
| PATCH | /me | 105 | 自分のみ | 60/min。`ProfileUpdateRequest` allowlist（`schemas/profile.py`）でフィールドを制限。**`real_name`/`student_number`/`birth_date` はスキーマから除外済み（KYC フィールド・upload-student-id 経由のみ・2026-06-06 #1）**。`interests`(要素50字・20件)・`clubs`/`hidden_clubs`(要素50字・5件)。gender/interest_in は設定後変更不可。identity_verified 後は faculty/department/admission_year 変更無視。テキストフィールド（name/bio/faculty/department/hometown/status_message/club）は HTML タグ除去後に保存（2026-06-06 #4）。必須項目が揃うと profile_setup_completed=true |
| POST | /upload-student-id | 209 | 自分のみ | 5/hour。**2ファイル必須**（`file`=学生証・`id_doc_file`=写真付き身分証）。EXIF 削除（`_strip_exif`・fail-close 422）。student-ids バケット（`{uid}/student_id_{ts}.ext` / `{uid}/id_doc_{ts}.ext`）。**Form フィールド**: `student_type`（"undergrad"\|"grad"）・`admission_year`(int, ge=2000, le=2100)・`birth_date`・`faculty`(max=50)・`department`(max=100)・`gender`・`interest_in`。**受け付けないフィールド**: `year`（学年はプロフ編集 PATCH /me で可変・complete-onboarding ゲートで必須チェック）・`real_name`・`student_number`（非取得）。**書き込み列**: `student_id_image_path`・`student_id_submitted`=True・`id_doc_image_path`・`id_doc_submitted`=True・`student_type`・`admission_year`・`birth_date`・`faculty`・`department`・`gender`・`interest_in`。status → pending_review |
...
| POST | /complete-onboarding | 764 | 自分のみ | 必須項目・学生証提出をサーバー検証。ゲート: student_id_submitted・id_doc_submitted・**year（必須: PATCH /me で事前入力必要）**・name・bio・写真1枚以上（2026-06-23 year チェック追加） |
...
- **ボカし（2026-06-26）**: GET /profiles・/recommended・/{user_id}・GET /likes/received は、viewer が female かつ充実度 score<80 のとき `blur_targets_for()（core/like_blur.py）` でいいね関係（liker_ids / i_liked_ids）を取得し、対象ユーザーの `avatar_url=None・blurred=true` を返す（photos=[] は /profiles/{id} のみ）。マッチ済みは常にボカさない。フロントは `blurred=true` で ColorfulCard・LikesReceivedPage にすりガラスプレースホルダーを描画。BrowsePage/HomePage/LikesReceivedPage/ProfileEditPage に告知バナー表示（danger 枠）
...
|---|---|---|---|---|
| POST | / | 81 | — | ○ 身バレ防止 `is_hidden_from_viewer` → 404（block 403 より前）。双方向 block → 403（中立メッセージ）。60/min。足跡経由は在庫消費なし。非足跡経路で `send_regime(gender, interest_in)` で regime 判定 → `female_unlimited`: 常に許可 / `male_hetero`: `ensure+consume`・在庫0→400 / `same_sex`: score<70 で `ensure+consume`・在庫0→400、score≥70 で無制限。**approved 必須**（`get_approved_user`・2026-06-03） |
| GET | /quota | 284 | 自分のみ（`LIKE_QUOTA_ENABLED=false` の間は全員 `is_target=false` で返す） |
| GET | /stock | 395 | 自分のみ（regime/score/is_unlimited/quantity/recovery_per_day/bonus_80_granted/bonus_100_granted/unlock_threshold を返す。ensure+grant_pending_bonuses を兼ね日次回復・初回ボーナスがこの GET で発火） |
| GET | /today-count | 361 | 自分のみ |
| GET | /received | 383 | ○ 一覧 | ○ block + match 済み + 身バレ防止 `get_hidden_user_ids_for` 除外。viewer が female かつ score<80 のとき全 liker を `avatar_url=null/blurred=true` で返す（退会済みは除外） |
...
### 2.12 お知らせ (announcements.py, prefix `/api/announcements`, approved user)
| Method | Path | rate limit | 説明 |
|---|---|---|---|
| GET | / | 60/min | 自分に合致するお知らせ一覧（既読フラグ付き）。ターゲット判定は `_matches_announcement`（target_all → 全員、else: 非空軸 AND）|
| GET | /unread-count | 120/min | 未読お知らせ件数 `{unread_count: int}`。Layout.tsx ベルバッジ用 |
| POST | /read | 10/min | 自分宛て未読お知らせを一括既読化（upsert `announcement_reads`）。fail-open |

> メンテ申し送り: これら3エンドポイントはメンテモード中も通す allowlist 対象

### 2.13 管理者お知らせ (admin_announcements.py, prefix `/api/admin/announcements`, require_admin)

| Method | Path | rate limit | 説明 |
|---|---|---|---|
| POST | / | 30/min | お知らせ作成（title≤100 / body≤1000 / target_all + target_faculties/grades/genders）。created_by=admin id |
| GET | / | 60/min | お知らせ一覧（論理削除済み含む全件）|
| PATCH | /{id} | 30/min | お知らせ更新（is_deleted=false のみ対象）|
| DELETE | /{id} | 30/min | 論理削除（is_deleted=true + deleted_at）|

### 2.15 メンテナンス状態 (maintenance.py, prefix `/api/maintenance`)
| Method | Path | rate limit | 認証 | 説明 |
|---|---|---|---|---|
| GET | /status | 120/min | なし | メンテナンス状態 `{"maintenance": bool}` を返す。メンテ中でも allowlist で通過。フロント 30秒ポーリング用 |

### 2.16 管理者メンテナンス制御 (admin_maintenance.py, prefix `/api/admin/maintenance`, require_admin)
| Method | Path | rate limit | 説明 |
|---|---|---|---|
| GET | / | 60/min | 現在のメンテナンス状態 |
| POST | / | 30/min | `{enabled: bool}` でメンテナンス ON/OFF。`set_maintenance()` で DB 更新 + キャッシュ即時反映 |

### 2.14 管理者 (admin.py, prefix `/api/admin`, 全 admin)
| Method | Path | 行 | 説明 |
|---|---|---|---|
...
| GET | /stats | 303 | 集計統計。返却フィールド: total_users / pending_count / approved_count / rejected_count / total_matches / total_messages / total_reports（pending のみ） / active_today / inquiry_unread_count（2026-06-06 追加）。AdminTabBar バッジ用カウント源 |
...
| identity_verified / profile_completed / profile_setup_completed / student_id_submitted / id_doc_submitted / onboarding_completed | bool | フロー制御フラグ。id_doc_submitted=migration 061 |
| rejection_reason / reviewed_at | text / timestamptz | 審査 |
| admission_year | int | 本人確認フォームで取得（2026-06-23 migration 062 以降・旧: 027 で利用停止扱いだったが upload-student-id の form で再取得する設計に変更） |
| student_type | text | 'undergrad'（学部生）/ 'grad'（院生）/ NULL（未完了）。本人確認フォームで取得・変更不可運用。migration 062 追加 |
| free_slots | text | 25桁の '0'/'1' 文字列（月〜金×5週=25コマ）。NULL = 未設定。migration 064 追加 |
| height_cm | smallint | 身長 cm（140〜190）。NULL = 未設定。migration 065 追加 |
| body_type | text | slim / average / muscular / glamorous / chubby。NULL = 未設定 |
| blood_type | text | A / B / O / AB。NULL = 未設定 |
| sibling_rank | text | only / eldest_son / eldest_daughter / second_son / second_daughter / third_son / third_daughter / later。NULL = 未設定 |
| languages | text[] | 使用言語（複数可）: ja / en / zh / ko / fr / de / es / other |
| campus | text | toyonaka / suita / minoh。NULL = 未設定 |
| housing | text | alone / family / dorm / share。NULL = 未設定 |
| commute_time | text | le30 / le60 / le90 / le120 / le150 / gt150（分換算コード）。NULL = 未設定 |
| commute_means | text[] | 通学手段（複数可）: train / bus / bicycle / walk / motorbike / car |
| second_lang | text | de / fr / zh / es / ru / ko / it / other（第二外国語）。NULL = 未設定 |
| relationship_goal | text | marriage / partner / friend_first。NULL = 未設定 |
| marriage_intent | text | someday / not_now / unsure。NULL = 未設定 |
| preferred_age_band | text | older / younger / same / any。NULL = 未設定 |
| drinking | text | often / sometimes / no。NULL = 未設定 |
| smoking | text | no / yes / vape / not_around_others。NULL = 未設定 |
| mbti | text | 16 種 MBTI（INTJ〜ESFP）。NULL = 未設定 |
| love_type | text | `^[LF][CA][RP][OE]$` パターン（4文字ラブ言語コード）。NULL = 未設定 |
| zodiac | text | GENERATED ALWAYS AS STORED（birth_date から 12 星座英語名を自動算出）。birth_date IS NULL → NULL。書き込み不可 |
...
### 3.15 daily_metrics（049）
PK `(snapshot_date, metric_key)`、value bigint、created_at timestamptz。service_role 専用（RLS + GRANT）。
pg_cron `snapshot-daily-metrics`（UTC 15:05 = JST 0:05）が `public.snapshot_daily_metrics(前日 JST 日付)` を実行して upsert。関数は SECURITY DEFINER + search_path=public。

**記録する metric_key（全 22 件）**

| 分類 | metric_key | 内容 |
|---|---|---|
| 累計 | users_total | deleted 除く全ユーザー数 |
| 累計 | users_approved | status=approved |
| 累計 | users_pending | status=pending_review |
| 累計 | users_rejected | status=rejected かつ identity_verified=false（審査不合格） |
| 累計 | users_suspended | status=rejected かつ identity_verified=true（管理者 suspend） |
| 累計 | users_banned | status=banned |
| 累計 | users_approved_male | approved かつ gender=male |
| 累計 | users_approved_female | approved かつ gender=female |
| 累計 | matches_total | matches 全件 |
| 累計 | messages_total | messages 全件 |
| 累計 | likes_total | likes 全件 |
| 累計 | reports_pending | reports.status=pending |
| 累計 | inquiries_unread | inquiries.status=unread |
| 累計 | photos_pending | profile_images.status=pending |
| 増分 | signups_new | profiles.created_at の JST 暦日 = target |
| 増分 | approvals_new | identity_verified=true かつ reviewed_at の JST 暦日 = target |
| 増分 | matches_new | matches.created_at の JST 暦日 = target |
| 増分 | messages_new | messages.created_at の JST 暦日 = target |
| 増分 | likes_new | likes.created_at の JST 暦日 = target |
| アクティブ | active_24h | approved + last_seen_at >= スナップ時点 - 24h |
| アクティブ | active_7d | approved + last_seen_at >= スナップ時点 - 7d |
| アクティブ | active_30d | approved + last_seen_at >= スナップ時点 - 30d |

> β では記録のみ（可視化は後日 daily_metrics SELECT で随時出力可）。アクティブは `last_seen_at` 上書き仕様のため時間帯別は再現不可（別途 activity_log が必要）。

### 3.10b user_inventory（043 + 069）
PK `(user_id, item_type)`、quantity（`CHECK 0<=quantity<=10000`）、last_grant_date、bonus_80_granted（bool DEFAULT false）、bonus_100_granted（bool DEFAULT false）、created_at、updated_at。item_type 縦持ち（CHECK `IN ('like_stock')`）。`set_updated_at` トリガー流用。

**2026-06-25 いいね経済 regime 実装（CC指示②）による挙動変更:**
- 初期在庫: 10→**5**
- regime = `female_unlimited`（女→男 or 不明）: 在庫行を作らない。送信無制限。
- regime = `male_hetero`（男→女）: 初期5。日次回復: score<80→+0 / score≥80→+1 / score=100→+2。初回80%→+5ボーナス(`bonus_80_granted`)・初回100%→+10ボーナス(`bonus_100_granted`)。フラグで多重付与防止。
- regime = `same_sex`（同性志向）: 初期5・日次回復なし・score≥70で消費免除（在庫行は残る）。
- `ensure_like_stock(user_id, regime, score)` が lazy-init + 日次回復を担当。
- `grant_pending_bonuses(user_id, score)` が一回性ボーナスを担当（male_hetero のみ呼ぶ）。
- 発火箇所: GET /likes/stock（ensure+grant）/ PATCH /profile/me 成功後（ensure+grant・male_hetero のみ）。
- one-shot 投入で既存 male approved 全員に `quantity=10` 初期付与済み（migration 043 末尾）。新規ユーザーは lazy-init で quantity=5。
...
### 3.14 login_history（019・migration 054 で DROP 済み）
id、user_id、ip_address、user_agent、logged_in_at。 → migration 054 で DROP（書き込みコードがなく未使用だったため除去・2026-06-22）。

### 3.19 daily_questions（066）
日替わり二択問題マスタ。
...
| body | text | NOT NULL |
| options | jsonb | NOT NULL・CHECK: array・要素 2〜4 個（各要素: {key, label}） |
| is_active | boolean | NOT NULL DEFAULT true |
| created_at | timestamptz | NOT NULL DEFAULT now() |

RLS: service_role のみ全操作（migration 067 で GRANT ALL + "service_role full access" ポリシー追加）。当日問題は `(JST 通算日数) % (is_active 行数)` 番目の display_order 行。

### 3.20 daily_answers（066）
日替わり二択回答。1ユーザー1日1回のみ（UNIQUE(user_id, answer_date)）。

| カラム | 型 | 備考 |
|---|---|---|
| id | uuid | PK DEFAULT gen_random_uuid() |
| user_id | uuid | NOT NULL FK → profiles(id) ON DELETE CASCADE |
| question_id | uuid | NOT NULL FK → daily_questions(id) ON DELETE CASCADE |
| choice | text | NOT NULL |
| answer_date | date | NOT NULL |
| created_at | timestamptz | NOT NULL DEFAULT now() |

RLS: service_role のみ全操作（migration 067 で GRANT ALL + "service_role full access" ポリシー追加）。回答後の変更は不可（INSERT only 運用）。

### 3.18 app_settings（053）
アプリ全体のフラグを key-value で保持するテーブル。
...
RLS: service_role のみ全操作（GRANT ALL TO service_role + "service_role full access" ポリシー）。初期行: `('maintenance_mode', 'false')`。

### 3.16 announcements（052）
PK `id uuid DEFAULT gen_random_uuid()`。

| カラム | 型 | 備考 |
|---|---|---|
| id | uuid | PK |
| title | text | NOT NULL、≤100字（Pydantic + DB 上限なし） |
| body | text | NOT NULL、≤1000字 |
| target_all | bool | NOT NULL DEFAULT false |
| target_faculties | text[] | NULL 許容。空配列 = 全学部合致 |
| target_grades | int[] | NULL 許容。空配列 = 全学年合致 |
| target_genders | text[] | NULL 許容。空配列 = 全性別合致 |
| is_deleted | bool | NOT NULL DEFAULT false（論理削除）|
| deleted_at | timestamptz | NULL 許容 |
| created_by | uuid | NOT NULL（admin user id）|
| created_at | timestamptz | DEFAULT now() |
| updated_at | timestamptz | DEFAULT now() |

RLS: service_role のみ全操作。`idx_announcements_is_deleted_created_at`（is_deleted, created_at DESC）。

### 3.17 announcement_reads（052）
PK `(announcement_id, user_id)`（複合）。

| カラム | 型 | 備考 |
|---|---|---|
| announcement_id | uuid | FK → announcements(id) ON DELETE CASCADE |
| user_id | uuid | FK なし（profiles はソフトデリートのため CASCADE 不発）|
| read_at | timestamptz | DEFAULT now() |

RLS: service_role のみ全操作。`idx_announcement_reads_user_id`（user_id）。
...
| snapshot_daily_metrics | 呼び出し: pg_cron `snapshot-daily-metrics`（UTC 15:05 = JST 0:05）および手動バックフィル | daily_metrics に 22 metric_key を upsert（累計・増分・アクティブ）。ON CONFLICT DO UPDATE で冪等。（049・SECURITY DEFINER・search_path=public） |
...
| ~~login_history~~ | ~~自分の行を SELECT~~ → 050 でポリシー DROP・054 でテーブル DROP |
...
> ※ 追補: migration 043 は 2026-06-06・migration 050 は 2026-06-10 に prod 適用済み。本 baseline（2026-06-04 時点）の「043 prod 未適用」記述は解消済み。最新 CLEAN 確認は 2026-06-10（prod・check_rls_drift -Target prod）。
...
|---|---|---|---|---|
| ホーム | HomePage.tsx | GET /api/profiles/recommended, /matches/, /likes/quota, /likes/stock, /matches/unread-count | なし | ブロック・hide・match・身バレ防止 除外。ITEMSカード: is_unlimited→「無制限」/male_hetero→quantity+1日N回復/same_sex未解放→quantity+「70%で送り放題」。male_hetero score<100 と same_sex !is_unlimited のとき ink枠告知バナー。女性は LIKE_QUOTA_ENABLED=true 時のみ受信枠カード表示。おすすめカードは liker_ids にある場合ボカし（blurred=true）。STATS「新着いいね」= unread_likes_received（receiver_read_at IS NULL）タップ→/notifications。CTA も /notifications 遷移（CC指示⑧ 2026-06-26） |
| さがす | BrowsePage.tsx → ColorfulCard（HomePage おすすめと共有） | GET /api/profiles（検索バー bio + 詳細検索: 学年/文理/出身地/eq13項目/身長レンジ/languages・commute_means overlap/並び替え）, /api/profiles/hometowns, /likes/today-count, /likes/stock, POST /likes/ | なし（検索条件は全てサーバー適用・履歴は localStorage） | ブロック・hide・身バレ防止・LIKE_QUOTA_ENABLED 時のみ枠フィルタ + 全フィルタ（学年/文理/出身地/bio/eq13項目/高さレンジ/overlap2項目）。eq13: body_type/blood_type/zodiac/campus/housing/commute_time/mbti/drinking/smoking/relationship_goal/marriage_intent/preferred_age_band/second_lang。overlap（`ov` 演算子・OR 一致）: languages/commute_means。sort_by: last_seen（デフォルト）/year_asc/year_desc/created_desc。在庫バッジ: is_unlimited→「∞」/それ以外→「♡×N」/0→警告色。viewer が female+score<80 のとき liker_ids 含む相手に blurred=true |
| プロフィール詳細 | ProfileDetailPage.tsx（自分/他人 共通・isSelf 分岐） | GET /api/profiles/{id}, POST /likes/, /safety/* | なし | 双方向ブロック 403・身バレ防止 404。3段構成（カルーセル左右矢印+ドット・メイン写真先頭 / 名前ブロック=学年/文理/出身地 tag-pill / 詳細ブロック=自己紹介+登録日）。背景はユーザー固有色全面・円形アバター廃止・興味/サークル非表示・横長浮遊いいねボタン。viewer が female+score<80 のとき (liker_ids ∪ i_liked_ids) の相手は avatar_url=null/photos=[]/blurred=true |
| プロフィール編集 | ProfileEditPage.tsx | GET/PATCH /api/profile/me, /profile/photos* | なし | 自分のみ。`ProfileCompletenessBar`（sticky top-14・live score・regime別コピー・上位3アドバイス）をヘッダー直下配置。female+score<80→danger枠blur告知。male_hetero+score<100→ink枠告知。same_sex+score<70→ink枠告知 |
| マッチ | MatchesPage.tsx | GET /api/matches/, /matches/unread-count, POST /matches/confirm | なし | ブロック・match 除外。GET /api/matches/ は last_message/last_activity_at/unread_count を返す（2026-06-23 追加）。**「あなたへのいいね」セクション撤去（CC指示⑧ 2026-06-26）→ 通知タブに移設**。非表示はプロフィール詳細から操作 |
| いいね受信 | LikesReceivedPage.tsx（/likes/received） | GET /api/likes/received, POST /likes/received/confirm, /likes/ | なし | ブロック・match・身バレ防止 除外。viewer が female+score<80 のとき全 liker を blurred=true（退会済み除く）で返しすりガラスプレースホルダー表示・プロフィール遷移無効。**CC指示⑧以降は通知タブ（NotificationsPage）が主導線・本ページは補助** |
| 通知 | NotificationsPage.tsx | GET /api/notifications/, /matches/unread-count, /likes/received, /announcements, POST /notifications/{id}/read, /announcements/read, /likes/received/confirm, /likes/ | なし | from_user_id ブロック除外。お知らせは get_approved_user のみ（block_utils 不要）。**CC指示⑧ 2026-06-26: 通知タブに受信いいね一覧＋いいね返しUIを統合（blur/dismiss/confirm すべて同等）。通知タブ開封時に receiver_read_at を一括既読化（home unread_likes_received バッジを下げる）** |
...
| 設定 | SettingsPage.tsx | GET /api/profile/me, /safety/blocks, /safety/hides（件数バッジ用・length のみ）, /admin/pending, /likes/stock, PATCH /profile/me, DELETE /profile/me, POST /push/test | なし | 自分のみ。male_hetero score<100 のとき ink枠告知バナー（2026-06-26 追加）。ブロック/非表示/お問い合わせは専用ページへ入口リンク3カードで分離（顔は出さない・問い合わせはバッジなし）。GA 同意トグル（プライバシー設定カード内）を設置（撤回導線・analytics.ts setConsent 経路に統一）。アカウント削除確認は custom modal（card-bold・不透明）。 |
...
### 6.1 認証・オンボーディング
```
/signup → 18歳自己申告チェックボックス（必須・フロントのみ） → メール確認(Supabase Auth) → /check-email（再送 funnel）→ /setup/required
OnboardingGuard:
...
WebSocket /ws/chat/{match_id}（JWT は `Sec-WebSocket-Protocol` ヘッダで送信・マッチメンバー+ブロック確認）
  接続失敗時 → ポーリング fallback（POST /messages/ + GET /messages/{id}）
既読: POST /messages/{id}/read → messages.read_at を一括更新 → WS で read_receipt ブロードキャスト
既読表示（LINE方式・2026-06-22）: フロントは lastReadAt（相手最終既読時刻）を messages.read_at から復元し、自分メッセージのうち既読済み最後の1件にのみ「既読」を表示（✓は廃止）
```
...
privacy_purge バッチ(APScheduler 毎日 03:00 JST, core/privacy_purge.py)
  → approved 後5日: 学生証画像（Storage）+ real_name/student_number（profiles）を物理削除（起点: reviewed_at）。生年月日は保持。本名・学籍番号はフォームから廃止（Phase B-A 2026-06-22）のため新規ユーザーは NULL のまま [PP 第4条(2) 2026-06-22改定]
  → approved + reviewed_at=NULL の場合: submitted_at 起点で同等処理（2026-06-02 追加）
  → rejected 後31日: 同上 [PP 第4条(2) 2026-06-22改定]
  → メッセージ削除: 退会時に auth.users 物理削除→ matches/messages が CASCADE 即時削除（PP 第4条(3)）。バッチ内 `purge_deleted_user_messages()` は dead code として 2026-06-19 削除済み
  → email_hash（退会後30日/BAN継続中はBAN期間終了まで延長）: `purge_expired_blocks`（identity_block.py）が定期呼び出しで IBH の retain_until 期限切れ行を物理削除 [PP 第4条(3) 2026-06-22改定・旧「ハッシュ1年保持」を email_hash 30日/BAN延長に変更]
```
...
|---|---|---|---|---|---|
| メンテナンス中に一般ユーザーアクセスを遮断 | ○ MaintenanceWatcher（30秒ポーリング→/maintenance へ強制遷移） | ○ MaintenanceMiddleware（HTTP→503 / WS→close 1011）。allowlist: `/health` / `/api/maintenance/status` / `/api/admin/*` / お知らせ3本 | — | ✅ OK | フロントはUX（バイパスされてもバック側503が守る）。CORSより内側配置でpreflight OPTIONSを503にしない |
...
| WebSocket token のログ露出防止 | — | ○ JWT を `Sec-WebSocket-Protocol` ヘッダで送受信（`useChat.ts`/`ws.py`・2026-06-06 B-11） | — | ✅ OK | Render アクセスログへの JWT 平文記録を解消。[17.9] 解消 |
| 自由入力テキストの HTML タグ除去 | — | ○ PATCH /me の `_strip_html_tags`（`<[^>]*>` regex）でサーバー保存前に除去 | — | ✅ OK 2026-06-06 | bio/name/faculty/department/hometown/status_message/club 対象。React JSX 自動エスケープとの多層防御 |
| PATCH /me での KYC フィールド書き換え防止 | — | ○ `ProfileUpdateRequest` から `real_name`/`student_number`/`birth_date` を除外（Pydantic が無視・2026-06-06） | — | ✅ OK | これら3フィールドは `upload-student-id` 経由でのみ確定 |
| API ドキュメント（/docs /openapi.json）の非公開 | — | ○ `APP_ENV=production` 時に FastAPI の docs 無効化（2026-06-06） | — | ✅ OK | dev は従来どおり使用可。Render prod 環境変数に `APP_ENV=production` **追加済み（2026-06-10）** |
...
| 再登録ブロック（退会/BAN 後） | ○ `useProfile` / `SetupRequiredPage` が GET /me の 423 を検出 → `/blocked` へ navigate（setup 画面を描画させない）。`BlockedPage.tsx` で withdrawal=日付つき / ban=中立文のみ を出し分け | ○ **関所B（手前）**: `GET /api/profile/me` 冒頭で `get_block_info(email_hash)` → 423（2026-06-22）。**関所C（最終）**: `POST /api/profile/upload-student-id` でも同照合（2026-06-18）。承認時に `upsert_on_approve`・退会時に `set_retain_until_on_delete`・BAN 時に `set_permanent_on_ban`（IBH 先→profiles.status 後の順序・fail-close）。`get_block_info` 照合キー: `email_hash` 一本（student_number_hash 不使用）。fail-close: `get_block_info` 例外時は ban 扱い。`set_permanent_on_ban` 例外は admin.py が 500 で返す（ghost-ban 防止） | — | ✅ OK | `identity_block_hashes` テーブル（auth.users CASCADE 非依存）が SSoT。在籍中=retain_until NULL・退会後30日=retain_until 設定・BAN=is_permanent=true。ban レスポンスに日付・「退会」の語なし（情報量を分離）。**不変条件**: block 行（is_permanent=true もしくは retain_until IS NOT NULL）は必ず email_hash を持つ。approved 行の email_hash は NULL 可（照合対象外のため問題なし）。email_hash の retroactive backfill は実施しない・設計上不要（照合対象=block 行のみ） |
...
Supabase SQL Editor で番号順に手動実行（ORM なし）。冪等性あり。新規は `051_*.sql` から採番（050 まで使用済み・036 が重複番号）。
...
| 049 | daily_metrics | `daily_metrics(snapshot_date, metric_key, value)` テーブル作成・RLS 有効化（service_role 専用）・`snapshot_daily_metrics(date)` 関数（SECURITY DEFINER + search_path=public）・pg_cron `snapshot-daily-metrics`（UTC 15:05 = JST 0:05・前日 JST 日付を引数）登録。22 metric_key を ON CONFLICT DO UPDATE で upsert（冪等）。**dev 適用日: 2026-06-10（オーナー手動）** / **prod 適用日: 2026-06-10（オーナー手動・初回シード実行済み）** |
| 050 | security_hardening_final | (B-1/B-7) `enforce_university_email_domain` を OR REPLACE 更新（`lower(NEW.email) LIKE '%+%@%'` で plus-alias 拒否・`split_part(...,'@',2)` でドメイン完全一致）。authenticated RLS ポリシー9本を DROP（profiles/hides/blocks×2/message_reactions/notifications×2/login_history/inquiries）＝service_role 一本化を完徹。冪等。**dev 適用日: 2026-06-06（MCP 検証 CLEAN）** / **prod 適用日: 2026-06-10（オーナー手動・`check_rls_drift -Target prod` CLEAN 確認）** |
| 051 | identity_block_hashes | 再登録ブロック用ハッシュ永続保持テーブル（auth.users CASCADE 非依存・service_role only RLS）。バックフィル（profiles.student_number_hash が既存の approved/banned 行を ON CONFLICT DO NOTHING で INSERT）。**dev 適用済み 2026-06-18** / **prod 適用済み 2026-06-19**・バックフィル完了（dev 33件・prod 7件・profiles_approved_banned と完全一致） |
| 052 | announcements | `announcements`（お知らせ本体・論理削除・target_all/faculties/grades/genders）＋ `announcement_reads`（既読管理・複合 PK・CASCADE）テーブル作成。RLS 有効化 + service_role 全許可。冪等。**dev/prod 適用済み 2026-06-22**（オーナー確認） |
| 053 | app_settings | `app_settings(key text PK, value text, updated_at)` key-value テーブル。メンテナンスフラグ等のアプリ設定保持。RLS 有効化 + service_role 全許可ポリシー1本。初期行 `('maintenance_mode','false')`。**dev/prod 適用済み 2026-06-22**（オーナー確認） |
| 054 | drop_login_history | `login_history` テーブルを DROP（migration 019 作成済み・書き込みコードなし・コード参照ゼロ確認）。**dev 適用済み 2026-06-22** / **prod 適用済み 2026-06-22（オーナー手動）** |
| 055 | drop_looking_for | `profiles.looking_for` カラムを DROP（フロント/バック参照を全除去後に適用）。**dev 適用済み 2026-06-22** / **prod 適用済み 2026-06-22（オーナー手動）** |
| 056 | drop_hash_columns | `profiles.real_name_hash` / `student_number_hash` の DROP（SSoT = identity_block_hashes・カバレッジ 100% 確認済み）。**dev 適用済み 2026-06-22** / **prod 適用済み 2026-06-22（オーナー手動・schema inspection で列消滅確認）** |
| 057 | add_email_hash_to_identity_block_hashes | `identity_block_hashes.email_hash` カラム追加（NULL 可・部分一意インデックス `uq_ibh_email_hash WHERE email_hash IS NOT NULL`）。照合キーを email_hash 一本化する Phase A の準備。**retroactive backfill は実施しない・設計上不要**（照合対象=block 行のみ。email_hash は BAN/退会/承認時に遅延付与）。**不変条件**: block 行（is_permanent=true もしくは retain_until IS NOT NULL）は必ず email_hash を持つ。prod 実測 2026-06-22: IBH 全行 email_hash NULL・permanent=0・退会ブロック=0（現時点で穴ゼロ）。**dev 適用済み 2026-06-22** / **prod 適用済み 2026-06-22（schema inspection で ibh.email_hash 存在確認）** |
| 058 | allow_null_student_number_hash | `identity_block_hashes.student_number_hash NOT NULL` 制約を DROP NOT NULL で解除。未承認退会ユーザーが email_hash のみで IBH に INSERT できるよう変更（旧制約で未承認退会 IBH 未記録 + 宙吊り4件が発生していた根本原因の一つ）。**dev 適用済み 2026-06-22** / **prod 適用済み 2026-06-22（schema inspection で is_nullable=YES 確認）** |
| 059 | drop_identity_plaintext_and_hashes | `profiles.real_name` / `student_number` と `identity_block_hashes.student_number_hash` / `real_name_hash` を DROP。IBH の照合キーは email_hash 一本（Phase A 以降）のため、不要になった平文・旧ハッシュ列を完全除去。Phase C-1 コード変更でコード側参照をゼロにした後に適用。**dev 適用済み 2026-06-22（MCP 経由・schema inspection で全列消滅・IBH 不変条件違反 0 件確認）** / **prod 適用済み 2026-06-22（オーナー手動）** |
| 060 | extend_year_range | `profiles.year` の CHECK 制約を `year <= 6` → `year <= 11` に拡張（院生対応: 7=修士1年・8=修士2年・9=博士1年・10=博士2年・11=博士3年）。`@ecs.osaka-u.ac.jp` は院生も終生有効なため登録可能に。アプリ層（`schemas/profile.py`・`routers/profile.py`）も `le=11` に更新済み。冪等（DROP CONSTRAINT IF EXISTS + ADD CONSTRAINT）。**dev 未適用（オーナー手動待ち）** / **prod 未適用（オーナー手動待ち）** |
| 061 | add_id_doc | `profiles` に `id_doc_image_path text`・`id_doc_submitted boolean NOT NULL DEFAULT false` を追加（本人確認2枚化: 写真付き身分証）。バケットは既存 student-ids を流用（パス `{uid}/id_doc_{ts}.ext`）。バックエンド実装はブロック2（2026-06-23 `py_compile` PASS）・フロント実装はブロック3と**同時デプロイ必須**。冪等（ADD COLUMN IF NOT EXISTS）。**dev 未適用（オーナー手動待ち）** / **prod 未適用（オーナー手番待ち）** |
| 062 | add_student_type | `profiles` に `student_type text`（NULL 許容・CHECK: undergrad/grad/NULL）を追加。既存行を `year <= 6 → 'undergrad'`, `year >= 7 → 'grad'`, `year IS NULL → NULL` で後埋め。**dev 適用済み 2026-06-23（MCP）**: column=text/YES確認・undergrad=42件/NULL=7件（year=NULL未完ユーザー）/grad=0件（現時点dev院生なし）。**prod 未適用（backend/フロント同時デプロイ時に当てる）** |
| 063 | realtime_broadcast | **dev 限定・prod には適用しない。** `realtime.messages` に受信ポリシー `rt receive own user channel`（authenticated が自分の `user:{uid}` トピックのみ受信可）を追加。`public.app_notify(target_ids uuid[], kind text)` ヘルパ関数を追加（security definer・service_role に EXECUTE 権限）。**dev 未適用（オーナーが dev Supabase SQL Editor で手動実行）** ⚠️ |
| 064 | add_free_slots | `profiles` に `free_slots text`（NULL 許容・CHECK: `^[01]{25}$` または NULL）を追加。月〜金×5週=25コマを '0'/'1' 文字列で表現。アプリ層制約: `ProfileUpdateRequest.free_slots` で `pattern=r"^[01]{25}$"` を Pydantic 検証。**dev 適用済み** |
| 065 | profile_fields | `profiles` にプロフ拡充 16 列（height_cm smallint / body_type・blood_type・sibling_rank・campus・housing・commute_time・second_lang・relationship_goal・marriage_intent・preferred_age_band・drinking・smoking・mbti・love_type text / languages・commute_means text[]）＋ zodiac text GENERATED ALWAYS AS STORED（birth_date から 12 星座英語名）を追加。全列 additive・NULL 許容。Pydantic 制約: ProfileUpdateRequest に Literal 型で値セット担保（zodiac は書き込み不可・ProfileUpdateRequest に含まない）。**dev 適用済み** |
| 066 | two_Question | `daily_questions`（問題マスタ）/ `daily_answers`（回答・1日1回）テーブル作成。RLS 有効化。seed 3問挿入。**dev 適用済み** / prod 未適用（オーナー手動待ち） |
| 067 | daily_grants_rls | `daily_questions` / `daily_answers` に GRANT ALL TO service_role ＋ "service_role full access" ポリシー追加（066 の抜け是正）。冪等（DO $$ IF NOT EXISTS $$）。**dev 未適用（オーナー手動待ち）** / prod 未適用（オーナー手動待ち） |
| 068 | extend_bio_max | `profiles.bio` の CHECK 制約を `char_length(bio) <= 500` から `<= 1000` に拡張。**dev 適用済み 2026-06-25** / prod 未適用（オーナー手動待ち） |
| 069 | like_economy_bonus_flags | `user_inventory` に `bonus_80_granted boolean NOT NULL DEFAULT false` / `bonus_100_granted boolean NOT NULL DEFAULT false` を追加。いいね経済・充実度スコア 80%/100% 到達時の一回性ボーナス付与済みフラグ。RLS/GRANT 変更なし（service_role 一本のまま）。**dev 適用済み 2026-06-25（MCP 直接適用・bonus_80/100 両列を schema inspection で確認済み）** / prod 未適用（オーナー手動待ち） |
| 070 | admin_user_last_sign_in | `CREATE OR REPLACE VIEW public.admin_user_list_v AS SELECT p.*, u.last_sign_in_at FROM public.profiles p LEFT JOIN auth.users u ON u.id = p.id`。admin ユーザー一覧に auth.users.last_sign_in_at を追加。ビューは RLS 対象外・service_role に SELECT GRANT のみ付与。**dev 未適用（オーナー手動待ち）** / prod 未適用（オーナー手動待ち） |
...
| REALTIME_BROADCAST_ENABLED | Phase 0 Realtime Broadcast の ON/OFF。**dev のみ `true`・prod は未設定=False**。True にすると `POST /api/messages/` 成功後に `public.app_notify` RPC でチャット相手の `user:{id}` チャンネルへ `{ kind: 'message' }` の合図を送信する。False（既定）では `notify_users()` は即 return（DB 呼び出しなし）。migration 063 が dev に適用済みであること前提 | `false`（未設定）|
| ADMIN_EMAILS | 管理者メール（カンマ区切り・小文字比較） | a@ecs.osaka-u.ac.jp,b@... |
| APP_ENV | アプリ環境識別子。`production` を設定すると `/docs`・`/redoc`・`/openapi.json` を無効化し HSTS ヘッダーを付与。未設定時は `development` 相当（docs 有効・HSTS なし） | `production`（**Render prod 設定済み 2026-06-10**） |
...
| VITE_REALTIME_ENABLED | Phase 0 Realtime Broadcast の ON/OFF（フロント側）。**dev Vercel Preview / ローカルのみ `true`・prod は未設定=false**。`true` のとき `Layout.tsx` がログイン済みユーザーの `user:{id}` プライベートチャンネルを購読し、`kind: 'message'` 受信で `['unread-count']`・`['matches']` を invalidate してリロードなし即時反映する。`false` では購読も購読解除も走らない |
