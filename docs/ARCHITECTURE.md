...
| POST | /upload-student-id | 209 | 自分のみ | 5/hour。2ファイル受付（`file`=学生証・`id_doc_file`=身分証、両方必須）。EXIF 削除（`_strip_exif`）。student-ids バケット（`{uid}/student_id_{ts}.ext` / `{uid}/id_doc_{ts}.ext`）。Form に `faculty`(max=50)・`department`(max=100)・`student_type`（undergrad/grad）・`admission_year`(ge=2000,le=2100)・`birth_date`(任意)・`gender`・`interest_in`。**`year` は本人確認フォームから削除（2026-06-23）。`year` はプロフ編集（PATCH /me）で別途入力・complete-onboarding ゲートで必須チェック。** **`real_name`・`student_number` は Phase B-A（2026-06-22）でフォーム廃止・受付削除。** status→pending_review。id_doc_submitted=True（2026-06-23 migration 061）。student_type/admission_year を profiles に書き込み（2026-06-23 migration 062） |
