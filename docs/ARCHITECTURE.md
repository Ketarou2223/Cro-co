...
| 051 | identity_block_hashes | 再登録ブロック用ハッシュ永続保持テーブル（auth.users CASCADE 非依存・service_role only RLS）。バックフィル（profiles.student_number_hash が既存の approved/banned 行を ON CONFLICT DO NOTHING で INSERT）。**dev 適用待ち** / **prod 適用待ち** |
| 052 | announcements | `announcements`（お知らせ本体・論理削除・target_all/faculties/grades/genders）＋ `announcement_reads`（既読管理・複合 PK・CASCADE）テーブル作成。RLS 有効化 + service_role 全許可。冪等。**dev 適用待ち** / **prod 適用待ち** |
| 053 | app_settings | `app_settings(key text PK, value text, updated_at)` key-value テーブル。メンテナンスフラグ等のアプリ設定保持。RLS 有効化 + service_role 全許可ポリシー1本。初期行 `('maintenance_mode','false')`。**dev/prod 適用待ち** |
| 054〜 | 未追加 | — |
