...
privacy_purge バッチ(APScheduler 毎日 03:00 JST, core/privacy_purge.py)
  → approved 後5日: PII 削除・ハッシュ保持（起点: reviewed_at）[PP v2.1 第4条(2)]
  → approved + reviewed_at=NULL の場合: submitted_at 起点で同等処理（2026-06-02 追加）
  → rejected 後31日: 同上 [PP v2.1 第4条(2)]
  → メッセージ削除: 退会時に auth.users 物理削除→ matches/messages が CASCADE 即時削除（PP v2.1 第4条(3)）。バッチ内 `purge_deleted_user_messages()` は dead code として 2026-06-19 削除済み
  → purge 後1年: ハッシュ削除
```
