...
[ Supabase ]  PostgreSQL + Auth + Storage
   - Storage: student-ids (Private) / profile-images (現状 Public)
...
├── core/  config.py / supabase_client.py / block_utils.py / image_utils.py /
...
|---|---|---|---|---|---|
| GET | /profiles | 47 | ○ 一覧 | ○ 双方向 block + hide を `not_.in_` で除外 | ○ faculty_hide_level / hidden_clubs を Python 側で適用 |
| GET | /profiles/recommended | 236 | ○ 一覧 | ○ 双方向 block + hide + match 済み除外 | ✗ **未適用**（要監査） |
| GET | /profiles/views | 354 | ○ 一覧（足跡） | ○ `get_blocked_user_ids` で除外 | ✗ 未適用（要監査） |
| POST | /profiles/views/confirm | 428 | 自分のみ | — | — |
| GET | /profiles/completeness-rank | 456 | 集計のみ | — | — |
| GET | /profiles/{user_id} | 517 | ○ 単一 | ○ 双方向 block → 403 / status≠approved → 404 | ✗ 未適用（要監査） |

- `/profiles` は 30/min。男性が女性一覧を見る場合のみ like_quota（used_count<5 かつ opens_at 到来）で受信可能な相手に絞る
- `/profiles/{user_id}` は閲覧時に足跡（profile_views）を upsert する

### 2.4 いいね (like.py, prefix `/api/likes`, 全 active)
| Method | Path | 行 | 他者情報 | ブロックフィルタ |
|---|---|---|---|---|
| POST | / | 71 | — | ○ 双方向 block → 403（中立メッセージ）。60/min |
| GET | /quota | 275 | 自分のみ | — |
| GET | /today-count | 352 | 自分のみ | — |
| GET | /received | 374 | ○ 一覧 | ○ block + match 済み除外 |
| POST | /dismiss/{liker_id} | 459 | — | — |
| POST | /received/confirm | 486 | 自分のみ | — |
...
PK `(user_id, date)`、opens_at、used_count、created_at。pg_cron `generate-like-quota` が毎日 UTC 15:00（JST 0:00）に女性×interest_in=male×approved の翌日枠を生成。
...
| blocks | `blocks_self`: blocker_id のみ全操作（038 で blocked_id の DELETE を塞いだ） |
...
|---|---|---|---|---|
| ホーム | HomePage.tsx | GET /api/profiles/recommended, /matches/, /profiles/completeness-rank, /likes/quota | なし | ブロック・hide・match 除外（身バレ防止は無し） |
| さがす | BrowsePage.tsx | GET /api/profiles, /likes/today-count, POST /likes/ | なし | ブロック・hide・身バレ防止・枠フィルタ |
| プロフィール詳細 | ProfileDetailPage.tsx | GET /api/profiles/{id}, POST /likes/, /safety/* | なし | 双方向ブロック 403（身バレ防止は無し） |
...
| いいね受信 | LikesReceivedPage.tsx | GET /api/likes/received, POST /likes/received/confirm, /likes/ | なし | ブロック・match 除外 |
| 通知 | NotificationsPage.tsx | GET /api/notifications/, /matches/unread-count, POST /notifications/{id}/read | なし | from_user_id ブロック除外 |
| 足跡 | FootprintsPage.tsx | GET /api/profiles/views, POST /profiles/views/confirm, /likes/ | なし | ブロック除外（身バレ防止は無し） |
...
| **同じ学部・学科の除外（身バレ防止）** | — | ○ **GET /api/profiles のみ** | — | ⚠️ 要監査 | recommended / 詳細 / 足跡 / いいね受信に未適用 |
| **同じサークルの除外（身バレ防止）** | — | ○ **GET /api/profiles のみ** | — | ⚠️ 要監査 | 同上 |
| プロフィール写真バケットの非公開化 | — | ○ コードは署名URL対応 | — | ⚠️ 要監査 | バケットが Public のまま（Private 化が残り） |
| WebSocket token のログ露出防止 | — | ✗ 未対応 | — | ⚠️ 要監査 | クエリパラメータが Render ログに残りうる |

### ⚠️ 要監査の要点
1. **身バレ防止（学部・サークル）は一覧専用**。`list_profiles`（browse.py:165-176）でのみ Python フィルタが効く。`get_recommended`・`get_profile`（詳細）・`get_profile_views`（足跡）・`get_received_likes`（いいね受信）には未実装。直リンクや足跡・おすすめ経由で、隠したい相手に見えてしまう可能性がある。
2. **profile-images バケットが Public**。署名付き URL に切替済みだが、元の公開 URL を知っていれば認証なしで取得できる状態。
3. **WebSocket token のログ露出**。`/ws/chat/{id}?token=JWT` の token が Render アクセスログに残りうる。
...
Supabase SQL Editor で番号順に手動実行（ORM なし）。冪等性あり。新規は `040_*.sql` から採番。
...
> **適用状況の追跡**: dev には 001〜038 適用済み（2026-05-25 時点）。039 を含む各環境への適用は手動。新規マイグレーション追加時は dev / 本番それぞれの適用日をこの表に追記すること。
> **036 は番号重複**（admin_extensions と profile_images_status）。新規採番では 040 以降を使う。
