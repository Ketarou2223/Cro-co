...
| API ドキュメント（/docs /openapi.json）の非公開 | — | ○ `APP_ENV=production` 時に FastAPI の docs 無効化（2026-06-06） | — | ✅ OK | dev は従来どおり使用可。Render prod 環境変数に `APP_ENV=production` 追加要 |
...
| APP_ENV | アプリ環境識別子。`production` を設定すると `/docs`・`/redoc`・`/openapi.json` を無効化し HSTS ヘッダーを付与。未設定時は `development` 相当（docs 有効・HSTS なし） | `production`（Render prod のみ設定） |
