...
| 044 | fix_rls_policies | 非 service_role ポリシー4本を DROP（再作成なし）: `hide_messages_with_deleted_user`〔messages・roles=public・anon 全件読み〕/ `match participants can view messages`〔messages・authenticated・prod 手動〕/ `blocks_delete_own`〔blocks・authenticated DELETE・ブロック解除バイパス〕/ `reports_self`〔reports・authenticated ALL〕。フロントは supabase.from 非使用のため再作成不要（service_role 一本化）。冪等。**dev 適用日: 2026-06-02（MCP apply_migration）** / ⚠️ **prod 適用日: 未適用（オーナー手動待ち）** |
| 045〜 | 未追加 | — |
