...
| POST | /upload-student-id | 209 | 自分のみ | 5/hour。2ファイル受付（`file`=学生証・`id_doc_file`=身分証、両方必須）。EXIF 削除（`_strip_exif`）。student-ids バケット（`{uid}/student_id_{ts}.ext` / `{uid}/id_doc_{ts}.ext`）。Form に `faculty`(max=50)・`department`(max=100)・`year`(ge=1,le=11)・`birth_date`(任意)・`gender`・`interest_in`。**`real_name`・`student_number` は Phase B-A（2026-06-22）でフォーム廃止・受付削除。** status→pending_review。id_doc_submitted=True（2026-06-23 migration 061） |
