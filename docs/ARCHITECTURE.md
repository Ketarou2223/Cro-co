...
| POST | /upload-student-id | 209 | 自分のみ | 5/hour。EXIF 削除（`_strip_exif`・fail-close 422）。Form に `faculty`(max=50)・`department`(max=100)・`year`(ge=1,le=6)・`birth_date`(任意)・`gender`・`interest_in` の制約付き。**`real_name`・`student_number` は Phase B-A（2026-06-22）でフォーム廃止・受付削除。** status→pending_review。student-ids バケット |
