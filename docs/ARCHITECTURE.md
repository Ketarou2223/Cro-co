...
```
backend/app/
├── main.py                  # FastAPI app・ルーター12本登録・APScheduler・CORS・セキュリティヘッダー（X-Frame-Options/nosniff/XSS-Protection/Referrer-Policy/Permissions-Policy）・HSTS（prod のみ）・BodySizeLimitMiddleware・APP_ENV=production 時に /docs /redoc /openapi.json を無効化
