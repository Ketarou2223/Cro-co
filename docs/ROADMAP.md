...
最終更新日: 2026-06-08（md 全体最新化・カテゴリ1〜10 アーカイブ整理）
...
- [ ] **looking_for カラムの DB 削除**: 現状フロント未使用。**β後送りで確定（2026-06-10）**（マイグレーション必要）
...
| 12.3 | 🟡 | 18歳未満排除が frontend + 自己申告で実装 | 🔵 一部対応（2026-06-04）: 法第10条の「明示」義務 → LandingPage（HERO + フッター）と SignupPage（フォーム内）に「18歳未満は利用できません」を明示済み。方針確定（2026-06-10）: 登録ページの自己申告チェックボックス方式で対応（⚠️ 実機未検証・Part 2 で実装）。backend 生年月日ガードは本番前の任意強化に降格（birth_date 収集済みのため後付け可能）。多層防御の根拠: 学籍メール＋学生証審査の組み合わせで実質的な排除は担保。届出・閲覧防止措置は 12.1 の管轄 |
