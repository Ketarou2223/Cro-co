...
- [ ] 非表示一覧ページ新設
- [ ] ブロック一覧を別ページへ分離
- [ ] プロフィール見え方改善（探索条件は文理化済み。**プロフィール表示の学部学科文理化は下記の別タスク参照**）
...
- [ ] **プロフィール表示の文理化**: プロフィール詳細・カードで学部学科をそのまま見せている箇所を文理表示に変える（探索条件は文理化済みだが表示は未対応）。**β前必須? β後送り? → オーナー判断待ち**（身バレ低減の一貫性を取るなら β前推奨だが、表示変更は影響範囲が広い）
...
- 非表示一覧ページ新設
- ブロック一覧を別ページへ分離
- プロフィール見え方改善（プロフィール表示の文理化は別タスク・セクション5 参照）
...
**作業項目:**
- ランディングページの目立つ位置に「β版」表示
- 初回登録フローの最初の画面に「β版・予期せぬ不具合の可能性」を表示
- 同意チェックボックスは置かない

**完了条件:**
- ランディングと初回登録の両方で β 表示が確認できる
- 文面が過度に不安を煽らない（さらっとした表現）
- スクショで両方の画面を記録
